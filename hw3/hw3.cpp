...
map<pair<char*, char*>, int> ip_pair_sum;
...
void dump_ip(u_int32_t length, const u_char *content, int type)
{
    ...
    char *source_ip = ip_ntoa(&ip->ip_src, type);
    char *destination_ip = ip_ntoa(&ip->ip_dst, type);
    ...
    pair<char*, char*> tmp = make_pair(source_ip, destination_ip);
    printf("%s    %s  \n", tmp.first, tmp.second);
    if (ip_pair_sum.find(tmp) == ip_pair_sum.end()) {
        ip_pair_sum[tmp] = 1;
    }
    else {
        ip_pair_sum[tmp] = ip_pair_sum[tmp] + 1;
    }
